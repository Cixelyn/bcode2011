...
//Justin's Go Here
	...
	public static MapLocation attackDebris(RobotPlayer myPlayer) throws Exception
	{
		...
		if ( rockMin != null )
		{
			...
			{
				gun =  myPlayer.myHammers[j];
				if ( !gun.isActive() && gun.withinRange(rockMin.location) && rockMin.robot.getRobotLevel() == RobotLevel.ON_GROUND )
					gun.attackSquare(rockMin.location, RobotLevel.ON_GROUND);
			}
