...
public class ReaperBehavior extends Behavior
{
	...
	boolean attackingRefinery=false;
	RobotInfo rInfo;
	Robot currentRefinery;
	
	private enum ReaperBuildOrder
	{
		EQUIPPING,
		DETERMINE_SPAWN,
		LOOK_FOR_REFINERIES,
	...
	ReaperBuildOrder obj = ReaperBuildOrder.EQUIPPING;
	...
	private static final int[] componentLoadOut = Utility.countComponents(new ComponentType[]    
                               {ComponentType.BEAM,ComponentType.SMG,ComponentType.SMG, ComponentType.SMG, ComponentType.SHIELD,
								ComponentType.JUMP,ComponentType.JUMP,
								ComponentType.SATELLITE,
								});
	
	public ReaperBehavior(RobotPlayer player)
	{
		super(player);
	}
	...
	public void run() throws Exception
	{
		...
		switch (obj)
		{
			...
			case EQUIPPING:	
				...
				{
					obj = ReaperBuildOrder.DETERMINE_SPAWN;
				}
				return;
			...
			case DETERMINE_SPAWN:
				...
				permRally = rally;
				obj = ReaperBuildOrder.LOOK_FOR_REFINERIES;
				return;
				
			case LOOK_FOR_REFINERIES:	
	        	...
	        	if (attackingRefinery) {
	        		int totalX=0;
	        		int totalY=0;
	        		for (int i=0; i<robots.length;i++) {
	        			if (!robots[i].getTeam().equals(myPlayer.myRC.getTeam())) {
	        				rInfo=myPlayer.mySensor.senseRobotInfo(robots[i]);
