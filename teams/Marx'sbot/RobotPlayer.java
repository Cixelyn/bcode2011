	...
	public void runBuilder(MovementController motor, BuilderController builder) {
		...
		while (true) {
            try {
				...
				else if(myRC.getTeamResources()>=(2*Chassis.LIGHT.cost+ComponentType.SMG.cost))
					builder.build(Chassis.LIGHT,myRC.getLocation().add(myRC.getDirection()));
					builder.build(ComponentType.SMG,myRC.getLocation(),RobotLevel.ON_GROUND);
