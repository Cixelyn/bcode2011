	...
	public void run() throws Exception {
		...
		switch (obj) {
			...
			case BATTLE_SITUATION: //we've seen enemies in this area, let's look around cause there will probably be more.
				...
				else if (lookAround==1) { //let's look to the right
					if (!myPlayer.myMotor.isActive()) {
						myPlayer.myMotor.setDirection(myPlayer.myRC.getDirection().rotateRight().rotateRight().rotateRight().rotateRight()); //have to go 180 degrees to compensate from previous left turn
						lookAround=lookAround+1;
					}
				}
				priorityAttack();
